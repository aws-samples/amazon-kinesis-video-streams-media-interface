...
//START AENC
int audioCapturerSetFormat(AudioCapturerHandle handle, const AudioFormat format,
		const AudioChannel channel, const AudioSampleRate sampleRate, const AudioBitDepth bitDepth)
{
...
int audioCapturerGetFormat(const AudioCapturerHandle handle, AudioFormat *pFormat,
		AudioChannel *pChannel, AudioSampleRate *pSampleRate, AudioBitDepth *pBitDepth)
{
	SV82X_HANDLE_NULL_CHECK(handle);
	SV82X_HANDLE_GET(handle);
...
int audioCapturerGetFrame(AudioCapturerHandle handle, void *pFrameDataBuffer,
